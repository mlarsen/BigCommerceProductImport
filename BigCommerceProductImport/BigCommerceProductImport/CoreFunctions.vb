...
Module CoreFunctions
    ...
    Public stFTPRequestRoot As String = "ftp://ftp.core-tech.com/dashboard/ChannergyTools/"
    Public stBuyNowURL As String
    Sub main()

    End Sub
    Function GetBuyURL(ByRef stApplication As String) As String
        Dim stSQL As String = "SELECT BuyURL FROM ChannergyTools WHERE Application='" + stApplication + "';"
        Dim da As New OdbcDataAdapter(stSQL, stODBCString)
        Dim ds As New DataSet()
        Dim dt As New DataTable
        Dim stBuyNowURL As String = ""
        Try
            da.Fill(ds, "Tools")

            For Each dr In ds.Tables("Tools").Rows
                stBuyNowURL = dr.item("BuyURL").ToString
            Next
            Return stBuyNowURL
    ...
    End Function
    Function GetFileVersion(ByRef stFilePath As String) As String
        Dim stVersion As String = FileVersionInfo.GetVersionInfo(stFilePath).ToString

        Return stVersion
    End Function
    Function IsLicensed(ByRef stApplication As String) As Boolean
        Dim stSQL As String = "SELECT IsLicensed FROM CTCtrl WHERE LicensedTo='" + GetCompany() + "' AND Application='" + stApplication + "';"
    ...
    Function IsNewVersion(ByRef stFTPVersion As String, ByRef stInstalledVersion As String) As Boolean
        ...
        Return bolIsNewer
    End Function
    Sub UpdateDialog(ByVal stMessage As String, ByRef textbox As Windows.Forms.TextBox)
        Dim CRLF As String = Chr(13) + Chr(10)
        If stMessage = "NULL" Then
            textbox.Clear()
        Else
            textbox.AppendText(TimeString + ": " + stMessage + CRLF)
        End If
    End Sub
    Function GetFTPFileVersion() As DateTime
        Dim ftp = FtpWebRequest.Create("ftp://ftp.braindoctor.net/pub/ChannergyTools/Channergy Dashboard.exe")
        ftp.Credentials = New NetworkCredential("mark@braindoctor.net", "@q]07V8Uv;B^$4")
        ...
        Return dte
    End Function
    Function GetFTPtxtVersion(ByRef stReadmeFileName As String) As String
        ...
        Try
            Dim ftp = FtpWebRequest.Create(stFTPRequestRoot + stReadmeFileName)
        ...
        Return stVersion
    End Function
    Function GetAddress(ByRef stDatapath As String) As String()
        Dim stIniFile As New IniFile()
        Dim stIniFilePath As String = stDatapath + "ChannergyGlobal.ini"
        Dim stAddress(6) As String

        If My.Computer.FileSystem.FileExists(stIniFilePath) = True Then
            stIniFile.Load(stIniFilePath)
            stAddress(0) = stIniFile.GetKeyValue("Company", "Company")
            stAddress(1) = stIniFile.GetKeyValue("Company", "Address1")
            stAddress(2) = stIniFile.GetKeyValue("Company", "Address2")
            stAddress(3) = stIniFile.GetKeyValue("Company", "City")
            stAddress(4) = stIniFile.GetKeyValue("Company", "State")
            stAddress(5) = stIniFile.GetKeyValue("Company", "Zip")
            stAddress(6) = stIniFile.GetKeyValue("Company", "Phone")
        End If
        Return stAddress

    End Function
    Function GetDataPath(ByRef stDataPath As String) As String
        Dim stLocalPath As String
        Dim stIniFile = New IniFile()
        Dim stIniFilePath As String = stDataPath + "Channergy.ini"
        ...
        If My.Computer.FileSystem.FileExists(stIniFilePath) = True Then
            stIniFile.Load(stIniFilePath)
            stLocalPath = stIniFile.GetKeyValue("Network", "NetworkDataDirectory")
    ...
    Sub LoadDataGridViewSQL(ByRef stSQL As String, ByRef GV As System.Windows.Forms.DataGridView)
        ...
        'Code added 01/13/2020: Set up wordwrapping in the DGV
        'GV.AutoSizeRowsMode = DataGridViewAutoSizeRowMode.AllCells
        GV.RowsDefaultCellStyle.WrapMode = Windows.Forms.DataGridViewTriState.True
